...
class MixShiftBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, shift_size, shift_dist, mix_size, layer_scale_init_value=1e-6,
        ...
        self.chunk_size = [i.shape[0] for i in jt.chunk(jt.zeros(dim), self.shift_size)]
    ...
    def execute(self, x):
        ...
        # split groups
        xs = jt.chunk(x, self.shift_size, 1) 

        # shift with pre-defined relative distance
        x_shift_lr = [ jt.roll(x_c, shift, 3) for x_c, shift in zip(xs, self.shift_dist)]
        x_shift_td = [ jt.roll(x_c, shift, 2) for x_c, shift in zip(xs, self.shift_dist)]
        ...
        x_lr = jt.concat(x_shift_lr, 1)
        x_td = jt.concat(x_shift_td, 1)
...
class PatchEmbed(nn.Module):
    ...
    def execute(self, x):
        ...
        if self.norm is not None:
            x = self.norm(x)
        return x.view(B, self.patches_resolution[0], self.patches_resolution[1], self.embed_dim).permute(0, 3, 1, 2)
