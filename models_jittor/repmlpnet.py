...
class RepMLPBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,
        ...
        if not deploy and reparam_conv_k is not None:
            for k in reparam_conv_k:
                ...
                # setattr(self, 'repconv{}'.format(k), conv_branch)          # jittor vision < 1.3.1.30
                self.__setattr__('repconv{}'.format(k), conv_branch)         # jittor vision >= 1.3.1.30
    ...
    def execute(self, inputs):
        ...
        if self.reparam_conv_k is not None and not self.deploy:
            ...
            for k in self.reparam_conv_k:
                conv_branch = self.__getattr__('repconv{}'.format(k))        # jittor vision >= 1.3.1.30
                # conv_branch = getattr(self, 'repconv{}'.format(k))         # jittor vision < 1.3.1.30
    ...
    def local_inject(self):
        ...
        if self.reparam_conv_k is not None:
            for k in self.reparam_conv_k:
                self.__delattr__('repconv{}'.format(k))     # jittor vision >= 1.3.1.30
                # delattr(self, 'repconv{}'.format(k))      # jittor vision < 1.3.1.30
    ...
    def _convert_conv_to_fc(self, conv_kernel, conv_bias):
        I = jt.init.eye(self.h * self.w).repeat(1, self.S).reshape((self.h * self.w, self.S, self.h, self.w))
        fc_k = nn.conv2d(I, conv_kernel, padding=(conv_kernel.size(2)//2,conv_kernel.size(3)//2), groups=self.S)
